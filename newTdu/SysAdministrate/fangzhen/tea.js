 ...
 function daoByClass() {
         ...
         var depertId=$('#departselection').val();
         var subjectId = $('.choosesub',parent.document)[0].id;
         var majorId=$('#classId').val();

         var that = this;
         //请求的路径
         var page_url = houtaiurl+"nanjingController/daoFangzhenByClass.action?subjectId="+subjectId+"&depertId="+depertId;
...
function getClass() {
     ...
     $.ajax({
         type:"POST",
         url:houtaiurl+"DepartmentController/GetMajorListByid.action?id="+ChooseSubject,
         async: false,
 ...
 function getClassReally() {
     var ChooseSubject=$('#classId').val();
     $.ajax({
         type:"POST",
         url:houtaiurl+"DepartmentController/GetClasslistBymajorid.action?id="+ChooseSubject,
         async: false,
         success:function(data){
             if(data.lenght>0){
                 classFlag=true;
             ...
             if(data){
                 ...
                 $('#classReally').html(html);
             }
         }
     })
 }

 var fangzhenFlag=false;

 function getFangzhenReally() {
     var subjectid=$('.choosesub',parent.document)[0].id;
     $.ajax({
         type:"POST",
         url:houtaiurl+"SubjectTreeController/GetSubjectTreePage.action?SubjectKey="+subjectid,
         async: false,
         success:function(data){
             if(data.lenght>0){
                 fangzhenFlag=true;
             ...
             if(data){
                 ...
                 var dataL=data.length;
                 for(var i=0;i<dataL;i++){
                     html+='<option value="'+data[i].knowledgecontentId+'">'+data[i].content+'</option>';
                 }
                 $('#fangzhen').html(html);
             }
         }
