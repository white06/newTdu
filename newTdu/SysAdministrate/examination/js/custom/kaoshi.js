...
$(function() {
    ...
    $.ajax({
        type: "POST",
        url: houtaiurl+"KaoshiController/timu.action?id=" + id,
            ...
            for(var i = 0; i < dataL; i++) {
                ...
                if(data[i].type == "单选题") {
                    ...
                    for(; j < neirongL; j++) {
                        ...
                        //var input = '<input type="text" style="width: 80%;height:30px;" class="tiankong" name="' + i + '_blanks_' + j + '" onchange="tiankongChange(this.name,\'' + data[i].neirong[j].questionKey + '\') ">'
                        var input = '<textarea type="text" style="width: 75%;height:70px;" class="tiankong" name="' + i + '_blanks_' + j + '" onchange="tiankongChange(this.name,\'' + data[i].neirong[j].questionKey + '\') "></textarea>'
                        var timu = tiankongXml(data[i].neirong[j].questionContent);
                        console.log(timu+"~~~~~~~~~~~~~~~~~")
                        //timu = timu.replace(/______/g, input);
                        html += '<div id="'+data[i].neirong[j].questionKey+'">'+
                    ...
                    console.log(herf+"----2")
                    console.log(head+"----3")
                    console.log(wendaHerf+"----4")
                    wendaHtml = head + html;
                    //console.log(wendaHtml)
                }
            }

            $.ajax({
                type: "POST",
                url: houtaiurl+"KaoshiController/getExamTime.action?examPage=" + id,
                success: function(data) {
                    data=120;
                    ...
                    $('#timu').html(headBotton + danxuanHtml + duoxuanHtml + tiankongHtml + panduanHtml+wendaHtml+"<p style='visibility:hidden;'>1</p>"+"<p style='visibility:hidden;'>1</p>"+"<p style='visibility:hidden;'>1</p>");
                    var herf = '<div class="rt_nr1_title">' +
                        ...
                        '</p>' +
                        '</div>';
                    $('.rt_nr1').html(herf + danxuanHerf + duoxuanHerf + tiankongHerf + panduanHerf+wendaHerf);
                    getTimeout();
                    seeTime();
                }
            });
...
function danpanChange(value, quesId) {
    $.ajax({
        type: "POST",
        url: houtaiurl+"KaoshiController/daan.action",
        data: {
            quesId: quesId,
            pageId: pageId,
            value: value
...
function danxuanXml(content) {
    ...
    var tigan_wenzi = $(xml).find("题干").text();
    var xuanxiang1_wenzi = $(xml).find("选项A").text();
    var xuanxiang2_wenzi = $(xml).find("选项B").text();
    var xuanxiang3_wenzi = $(xml).find("选项C").text();
    var xuanxiang4_wenzi = $(xml).find("选项D").text();
    var timu = [];
...
function duoxuanXml(content) {
    ...
    var xuanxiang3_wenzi = $(xml).find("选项C").text();
    var xuanxiang4_wenzi = $(xml).find("选项D").text();
    var xuanxiang5_wenzi = $(xml).find("选项E").text();
    var xuanxiang6_wenzi = $(xml).find("选项F").text();
    timu.push(tigan_wenzi);
    timu.push(xuanxiang1_wenzi);
    timu.push(xuanxiang2_wenzi);
    timu.push(xuanxiang3_wenzi);
...
function jiaojuan() {
	if(key == 0) {
		...
		$.ajax({
			type: "POST",
			url: houtaiurl+"KaoshiController/jiaojuan.action?pageId=" + pageId,
			success: function(data) {
				if(data == "ture") {
					alert("交卷成功");
...
function getfenshu() {
    $.ajax({
        type: "POST",
        url: houtaiurl+"KaoshiController/getfenshu.action?pageId=" + pageId,
        success: function(data) {
            console.log(data);
            alert("考试得分为："+data.fenshu);
        }
    })
}
...
function stu_getAnswer() {
	$.ajax({
		type: "POST",
		url: houtaiurl+"KaoshiController/seleAnswer.action?pageId=" + pageId,
		async: false,
		success: function(data) {
			if(data) {
				var dataL = data.length;
