...
function getQuestion(knowId){
	 $.ajax({
		 url: houtaiurl+ "ExercisesController/getAllQuestion.action",
         data: { knowId: knowId },
         type: "Post",
         async: false,
...
//0表示第一题
		if(i==0){
			...
			if(result[i].type=="单选题"){
				console.info(xml.getElementsByTagName("文字").length);
				//获取需要的值并写入html中展示出来
...
function shoucang(timuId){
	 $.ajax({
		 url: "../../../ExercisesController/addPersonal.action",
